...
| `STO-E011` | `LayoutMismatch` | A postgres open's DSN disagrees with the layout the memory already has (#353). Either the DSN names `?meta_schema=` (the **paired** layout: engine metadata — `meta`, `counters`, `ns_reg`, `telem_*` — in its own schema) but the memory schema already carries `meta` in-schema (the single-schema layout), or the DSN names no metadata schema but the memory schema holds `grains` without `meta`, which only a paired memory looks like. Proceeding would bootstrap a second, empty set of engine metadata and run against it — every legal hold, retention policy, saved query and trigger lease silently absent, two writers allocating ids from two counter rows — so the open is refused before any lock or DDL, on read-write, `provision=never` and read-only opens alike. A layout change is an explicit `ALTER TABLE … SET SCHEMA` migration (`docs/deployment-profile.md`, "Paired layout"), never something an open does by itself |
