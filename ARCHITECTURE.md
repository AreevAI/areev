...
| **areev-mcp** | cal, core, store, areev-loop, areev-loop-adapter | The stdio MCP server — eight memory- and improvement-semantic tools (six memory + `areev_loop_adapter` / `areev_recommendations`) over newline-delimited JSON-RPC 2.0. See the [MCP reference](docs/mcp-reference.md). |
