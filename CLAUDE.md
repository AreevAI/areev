...
| `areev-conformance` | Backend-parameterized conformance suite (`publish = false`) — one case list (forks, replication, tombstones, PITR, BM25, vectors, CAS, CAL smoke) run against BOTH backends; the Pg runner needs `DATABASE_URL`/`AREEV_PG_URL` and hard-fails when `CI=true` without one. `--features sandbox` adds the sandbox guest-ABI case (#340): the committed non-Rust modules through the `areev-sandbox` binary (`AREEV_SANDBOX`), same CI hard-fail rule | — |
