...
| [`blessed-tools/`](blessed-tools/) | **The tool gateway becomes configuration** — the shipped `http.call` blob bound by a pack with declared hosts: one permitted call (200, credential attached by the broker, never held by the tool), one call to an undeclared host refused with `RUN-E022`, and `tool provenance` chaining the blob. Offline, against a loopback stub |
| [`grain-connector/`](grain-connector/) | **The connector is in the memory, not on the box** — a polling trigger that names its connector by content address (`connector_tool`): pinned wasm, sandboxed, brokered, with no host connector script anywhere. Installs from a hand-written pack, runs keyless and offline, and shows the `TRG-E012` refusal a host that pinned nothing gets |
