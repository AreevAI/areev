...
| Dir | What |
|---|---|
| [`agents/`](agents/) | Vertical agents on triggers + `areev run`: work in — polled from a mailbox, or **pushed as a webhook your own listener delivers** — workflow, human approval (with corrections by email reply), system of record out; then the loop reads those runs back and proposes a fix a person approves. Stacks are one file per language (Python, TypeScript, Rust), all minting one shared content-addressed plan; zero repo dependencies — every vendor leg is a JSON-on-stdio connector or host tool |
