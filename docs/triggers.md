...
trigger — an as-of `entity_at`, a `related` walk or a bounded `recall`,
whichever host starts the run — is a declared [`reads`](run.md#reading-the-runs-own-memory-reads) entry,
