...
| **Workflow** | A plan: a directed graph of steps bound to tools, immutable and content-addressed, so every edit mints a new hash | Run state or trigger schedules — both point *at* the plan; it stores neither | CAL `ADD workflow "name" … graph` · JSON `add("workflow", …)` for bounded cycles, reducers and memory `reads` · the console canvas |
