...
| `namespace` | string | no | Defaults to the session namespace. Accepts a prefix scope `"org.*"` — the namespace `org` plus every `.`-descendant (`org.sales`, `org.sales.emea`); `organization` never matches, and `org*`/bare `*` refuse (`VAL-E001`). Writes never accept patterns, nor an unspellable name — whitespace, control or invisible formatting characters (`VAL-E001`), because a write is what mints a namespace and a typo in one is found by nothing. |
...
| Tool | What it does |
|---|---|
| `areev_run_start` | Start a run — `workflow` (64-hex), fresh `run_id`, optional `input` (any JSON), budgets (`max_tokens`, `max_usd_micros`, `max_wall_ms`, `max_supersteps`) and the abstract-node loop limits (`max_effects_per_attempt` — effects one node attempt may spend, its model turns, tool calls and re-prompts sharing the counter, default 16; `llm_tool_result_chars` — how much of ONE tool result the model is shown, in characters, default unbounded, with the journal keeping every result in full either way; `llm_context_tokens` — a ceiling on the WHOLE transcript in the provider's own reported prompt tokens, default none, past which the middle is folded into one journaled summary and `RUN-E024` fails the node when nothing is foldable). All three are frozen into the manifest, so a resume is bounded exactly as the start was. Client-gated nodes park the run and return a `requires_action` envelope. Code-carrying / sandboxed Definitions follow the same server-bound posture as `$AREEV_RUN_TOOL_CMD`: the operator sets `$AREEV_RUN_ALLOW_EXECUTOR` (the executor pin, comma list), `$AREEV_RUN_EXECUTOR_CACHE`, `$AREEV_RUN_SANDBOX_CMD` (the areev-sandbox runner for `runtime: "wasm32-areev"`), and `$AREEV_RUN_EXECUTOR_TIMEOUT` (#133 — overrides the fixed 300s wall-clock ceiling either executor otherwise runs a tool under; `0` waits forever) at server start — a client can never pin code, because the pin IS the authorization. `$AREEV_RUN_TOOL_ENV` (#188 — a comma list of variables a host tool may keep; set, the tool's environment is cleared down to those plus the minimal set a command needs to start) is server-bound the same way. **`initiator` is not accepted from a client** (1.9.0, #293): it is attribution the SERVER asserts about who a run is started for, and an identity a caller can claim about itself is not an identity — the same reasoning as the `responder` principal. The other 1.9.0 run knobs (`--initiator`, `--harness-ns`, `--allow-confirmation-asks`, `--input-placement`, `--lease`, `--node`, the concurrency caps) are host configuration, server-bound the same way. |
