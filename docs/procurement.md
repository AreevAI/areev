...
| Audit log, tamper-evident | ✅ | `areev audit export` — destruction trail + loop lifecycle as JSONL, **both** hash-chained (`derived_from` names the predecessor, `context.seq` makes a gap detectable) and verified on export; truncation is flagged, never silent. A window edge is reported as `previous_outside_window`, not as a break, so an hourly incremental import is not noisy; records written before chaining existed export as `unchained`. `--with-outcomes` adds the Verify gate's measured checkpoints, marked `chained: false` because they come from the rebuildable state blob |
| Data retention with floors | ✅ | `areev retention` declarative policies (file-truths); `--min-days` floors + legal holds refuse destruction of the logs the policy cites — **every** deletion path, not only the age-based ones (`STO-E009`), with the deferral itself recorded. Overriding a hold needs `admin` on the namespace and names the hold in the audit record |
...
| Observability / OTel | ✅ | OTLP/HTTP JSON span export (`--otel-endpoint`) — one trace per run, resumes join the same trace, spans carrying the **OpenTelemetry GenAI semantic conventions** (`gen_ai.operation.name` chat/execute_tool/invoke_agent, model, usage, finish reasons, tool call ids) so an existing GenAI backend reads them with no Areev-specific code; §6.10 event stream (`--events`). Attribute table in [`run.md`](run.md#watching-a-run) |
