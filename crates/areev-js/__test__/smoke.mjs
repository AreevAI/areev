...
test('onEvent streams the same JSON lines the CLI --events prints', async () => {
  ...
  const started = []
  // onEvent is the 17th argument — the 12 nulls stand in for the budget,
  // model and executor-pin parameters between toolCmd and it.
  const session = JSON.parse(await m.runStart(
  ...
  const done = JSON.parse(await m.runResume(
    'js-ev', null, null, null, null, null, null, null, null, null,
