...
The generated body is compared byte-for-byte, with ONE exception: a blockquote
written directly under the title is carried through regeneration instead of
being deleted by it. That is where a published run says the thing the directory
cannot derive — "the scorer moved after this ran, so these numbers are not
comparable to a re-run" — and it belongs next to the checksums, where someone
auditing the evidence will read it. It is prose, never a number: every file is
still checksummed, so a note cannot make a changed transcript verify.
...
def read_banner(run_dir):
    """A hand-written editorial note directly under the manifest's title.

    Everything else in this file is generated and compared byte-for-byte,
    which is the point — a published transcript that changed must fail the
    build. But a published run sometimes needs a NOTE that is not derivable
    from the directory: most importantly "the scorer moved after this ran, so
    these numbers are not comparable to a re-run". That note belongs in the
    manifest, where someone reading the checksums will see it.

    So one blockquote immediately after the `# …` title is carried through
    regeneration instead of being silently deleted by it. It is prose, never a
    number: every file in the directory is still checksummed, and a note
    cannot make a changed transcript verify.
    """
    ...
    for line in lines[1:]:
        if line.startswith(">"):
            seen, _ = True, banner.append(line)
        elif line.strip() == "":
            if seen:
                break
            banner.append(line)
    ...
    if not seen:
        return []
    # Normalize to exactly one blank line on each side of the quote.
    while banner and banner[0].strip() == "":
        banner.pop(0)
    return ["", *banner]
...
def main(argv):
    ...
    if write:
        # Build BEFORE opening for write: `open(..., "w")` truncates, and the
        # generator reads the existing file to carry its editorial banner
        # through. Inlined, regeneration would silently delete the note.
        text = build_manifest(run_dir, stems)
