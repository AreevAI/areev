...
- **A memory is a file path or a DSN.** `AREEV_BENCH_DB` (receipts also takes
  `run.py --db`; selfimprove takes `selfimprove_aba --db`) names it, and it is
  handed to `areev.Areev` verbatim, so a
  ...
  place the two are told apart. The Rust side spells the same rule as
  `selfimprove::memory::BenchDb` (#250): one resolve, one redact, and the
  workdir holds ARTIFACTS while `--db` decides only where the memory lives.
  What a schema cannot do is be COPIED: an arm
  ...
  arm on the one memory instead (`selfimprove_learn` refuses the same way). On Postgres a schema is the isolation unit
