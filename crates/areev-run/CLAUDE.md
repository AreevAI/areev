...
- The sandbox seam spawns under `EnvPolicy::ClearExcept`; native blobs and
  ...
  `$AREEV_RUN_TOOL_ENV` and the bindings' `tool_env`. It drops any name in the
  `SECRET_ENV` registry and returns it, so an allow list cannot re-admit a
  known secret; the broker's resolver seam re-admits on purpose, which is why
  the filter lives in the parser and not in `proc::run`. `deny_vars` leaves a
  `ClearExcept` policy alone — rebuilding it as `InheritExcept` would widen the
  child's environment while looking like a tightening. The sandbox path ignores
