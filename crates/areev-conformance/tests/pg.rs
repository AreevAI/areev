    ...
    areev_store::pg::execute_raw(
        ...
        &format!(
            "CREATE SCHEMA \"{schema}\"; \
             CREATE TABLE \"{schema}\".terms(id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, term text UNIQUE); \
    ...
    assert_eq!(
        q(format!(
            "SELECT count(*) FROM pg_constraint c JOIN pg_class t ON c.conrelid = t.oid \
             ...
             WHERE n.nspname = '{schema}' AND t.relname = 'terms' AND c.conname = 'terms_term_key'"
        )),
        0,
        "the text UNIQUE constraint — the ~2704-byte cap — is gone"
    );
